<template>
    <div class="header-wrap" :class="[visabilityHeader ? '' : 'header-wrap-press-up', showHeader ? '' : 'header-wrap-visability']">
        <v-navigation-drawer
            class="mobile-menu"
            v-model="drawer"
            fixed
        ...
        >
            ...
            <v-list-item-icon>
                ...
                <path d="M1.69531 1H38.3033" stroke="#0154A6" stroke-width="4" stroke-miterlimit="10" stroke-linecap="round"/>
                <path d="M1.69531 10H38.3033" stroke="#0154A6" stroke-width="4" stroke-miterlimit="10" stroke-linecap="round"/>
                <path d="M1.69531 19H38.3033" stroke="#0154A6" stroke-width="4" stroke-miterlimit="10" stroke-linecap="round"/>
                </svg>
            </v-list-item-icon>
        ...
        <header>
                    ...
                    <div class="upper-header-content">
                        ...
                        <client-only>
                            <span v-if="settings.email">
                                <a class="d-block d-sm-none" :href="settings.email.href">{{ settings.email.name }}</a>
                            </span>
                            <div>
                                <span class="d-none d-sm-inline" v-if="settings.email">
                                    <a :href="settings.email.href">{{ settings.email.name }}</a>
                                </span>
                                <span v-if="settings.phone">
                                    <a class="pl-14" :href="settings.phone.href">{{ settings.phone.name }}</a>
                                </span>
                            </div>
                        </client-only>
                    </div>
                    <v-divider class="header-line" v-show="!showHeader" color="white" insert></v-divider>
                </div>
            </div>
                    ...
                    <div class="bottom-header-content">
                        ...
                        <div class="d-none d-md-block">
                            <span @mouseover="expand = true">
                                <nuxt-link @click.native="expand = false" class="catalog-menu_btn" to="/product-category/">Каталог</nuxt-link>  
                            </span>
                            <nuxt-link @click.native="expand = false" class="pl-xl-15 pl-7" to="/service/">Сервис</nuxt-link>
                            <nuxt-link @click.native="expand = false" class="pl-xl-15 pl-7" to="/parts/">Запчасти</nuxt-link>
                            <nuxt-link @click.native="expand = false" class="pl-xl-15 pl-7" to="/lizing/">Лизинг</nuxt-link>
                            <nuxt-link @click.native="expand = false" class="pl-xl-15 pl-7" to="/about/">О компании</nuxt-link>
                            <!-- <nuxt-link @click.native="expand = false" class="pl-xl-15 pl-7" to="/blog/">Блог</nuxt-link> -->
                            <nuxt-link @click.native="expand = false" class="pl-xl-15 pl-7" to="/contacts/">Контакты</nuxt-link>
                        </div>
                        <div class="d-block d-md-none mobile-menu-icon" @click.stop="drawer = !drawer">
                            <svg width="40" height="20" viewBox="0 0 40 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M1.69531 1H38.3033" stroke="white" stroke-width="2" stroke-miterlimit="10" stroke-linecap="round"/>
                            <path d="M1.69531 10H38.3033" stroke="white" stroke-width="2" stroke-miterlimit="10" stroke-linecap="round"/>
                            <path d="M1.69531 19H38.3033" stroke="white" stroke-width="2" stroke-miterlimit="10" stroke-linecap="round"/>
                            </svg>
                        </div>
                    </div>
                    ...
                    <div v-show="expand" @mouseleave="expand = false" class="catalog-menu_wrapper">
                        ...
                        <v-container class="catalog-menu_container catalog-main_products menu-right_wrap">
                            ...
                            <v-row v-else>
                                <v-col cols="10">
                                    <v-row>
                                        <v-col v-for="item, index in naves" :key="index" cols="4">
                                            <nuxt-link @click.native="expand = false" :to="`/product-category/${item.url}`">
                                                {{ item.name }}
                                            </nuxt-link>
                                        </v-col>
            ...
            <client-only>
            <div v-if="displayBreadcrumbs" :class="[viasbilityBreadcrumbs ? 'show-breadcrumbs' : '', visabilityHeader ? 'breadcrumbs-item_colored' : '']" class="header-breadcrumbs container pt-9 pt-lg-11 ">
                <v-breadcrumbs itemscope itemtype="http://schema.org/BreadcrumbList" class="pa-0" :items="$store.getters.getBreadcrumbs">
                    <template v-slot:item="{ item }" >
                        ...
                        >
                            <nuxt-link v-if="!item.disabled" :to="item.href">{{ item.text }}</nuxt-link>
                            <span :class="visabilityHeader ? 'disabled-breadcrumb-black' : 'disabled-breadcrumb-white'" v-else>{{ item.text }}</span>
...
<style lang="scss" scoped>
    ...
    .header-wrap-visability .bottom-header, .header-wrap-visability .upper-header{
        background: unset;
    }
    .header-line {
        transition: .5s all;
...
<style lang="scss">
    ...
    .header-wrap-press-up .theme--light.v-breadcrumbs .v-breadcrumbs__divider, .theme--light.v-breadcrumbs .v-breadcrumbs__item--disabled {
