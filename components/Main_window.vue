<template>
  <div class="main-window-wrap">
    <div style="position: absolute; height: 100%; width: 100%">
        ...
        :show-arrows="false"
        >
            <v-carousel-item height="100%" v-for="slide, index in slides" :key="index">
                <img v-if="index > 0" :src="slide" loading="lazy"/>
                <img v-else :src="slide" loading="lazy"/>
            </v-carousel-item>
        </v-carousel>
        <v-carousel
        v-else
        ...
        :show-arrows="false"
        >
            <v-carousel-item height="100%" v-for="(slide, i) in slides" :key="i">
                <img
                    :src="slide"
                    loading="lazy"
                />
    ...
    <v-container class="main-window-content_wrap">
        ...
        <v-row class="main-window_content">
            <v-col lg="8">
                <h1 class="pb-10 pb-sm-15 pt-16" v-html="title">
                </h1>
                 <nuxt-link :to="link" v-if="link != ''">
                    <v-btn class="main-window_button text-none font-weight-bold" color="primary">
                        {{ btnTitle }}
                    </v-btn>
                </nuxt-link>
                <v-btn v-else class="main-window_button text-none font-weight-bold" color="primary" @click="$store.commit('onModalWindow', {title: btnTitle, btnName: btnTitle})">
                    {{ btnTitle }}
                </v-btn>
            </v-col>
