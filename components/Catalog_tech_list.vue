        ...
        >
          <nuxt-link class="card-link" :to="`/product/${item.url}/`">
            <div class="card-content">
              <div class="label-wrapper" v-if="item.labels.length != 0">
              ...
              <img v-if="item.images.length == 1" :src="item.images[0].url" :alt="item.images[0].alt" :title="item.images[0].title" loading="lazy"/>
              <div class="hover-slider" v-else-if="item.images.length > 1">
                <div class="hover-slide_wrapper">
                  <span @mouseover="hoverPhotoEvent(item, 0)" :class="item.hoverPhoto == 0 ? 'active-breadcrumb' : ''" class="hover-slide"></span>
                  <span @mouseover="hoverPhotoEvent(item, 1)" :class="item.hoverPhoto == 1 ? 'active-breadcrumb' : ''" class="hover-slide"></span>
                </div>
                <img class="hover-slider_image" 
                :class="
                  hoverCard == index ? 
                    hoverSinglePhoto == indexPhoto ? 'active' : 'inactive' 
                  : 
                    item.hoverPhoto == indexPhoto ? 'active' : 'inactive'" v-for="photo, indexPhoto in itemPhoto(item.images, 2)" :src="photo.url" :alt="photo.alt" :title="photo.title" loading="lazy"/>
              </div>
              ...
              <div class="card-inner-content">
                  ...
                  <div class="catalog-card__characteristics" v-if="item.mainCharacteristics">
              ...
              <v-row no-gutters dense>
                <v-col cols="6">
                  <nuxt-link :to="`/product/${item.url}/`">
                    <v-btn color="#C8C7C7" height="43px" class="text-none font-weight-bold more-link" block>
                      Подробнее
                    </v-btn>
                  </nuxt-link>
                </v-col>
                <v-col cols="6">
                  <v-btn color="primary" height="43px" class="text-none font-weight-bold" block @click="$store.commit('onModalWindow', {title: 'Узнать цену', btnName: 'Узнать цену', text: '<strong>' + item.name + '</strong>'})">
                    Узнать цену
                  </v-btn>
                </v-col>
              </v-row>
  ...
  computed: {
    mainCharacteristicsArray() {
      ...
      this.items.forEach(element => {
        ...
        this.category.product_main_characteristic_ids.forEach(mainCharCategory => {
