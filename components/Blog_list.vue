      ...
      <div class="block-padding" v-if="mainArticles.length != 0">
        <v-row class="mt-0 mb-md-8 mb-3">
          ...
          <v-col cols="12" sm="6" class="d-flex align-center justify-sm-end">
            <nuxt-link class="big-main-text href-primary text-none underline-href font-weight-bold" to="/blog/">Перейти в блог</nuxt-link>
          </v-col>
        </v-row>
        <v-row class="catalog-block">
          <v-col
            ...
            :key="index"
            :class="[index == 6 ? index = 0 : '', index == 0 || index == 4 ? 'col-sm-6 col-md-6 col-12' : 'col-sm-6 col-md-3 col-12']"
          >
            <v-card :class="index == 1 || index == 4 || index == 5 ? 'background-card' : 'card'">
              <img :src="index == 1 || index == 4 || index == 5 ? item.image_vertical.url : item.image_horizontal.url" 
      ...
      >
        <v-card :class="index == 1 || index == 4 || index == 5 ? 'background-card' : 'card'">
          ...
          <div class="content-card">
            <p class="font-weight-bold">{{ item.name | cutText(100, '...')}}</p>
            <p>{{ item.published_at | formatDate }}</p>
            <nuxt-link class="big-main-text underline-href text-none font-weight-bold" :to="item.url + '/'">Читать полностью</nuxt-link>
          </div>
        </v-card>
      </v-col>
    </v-row>
